...
The SDK includes an [MCP](https://modelcontextprotocol.io/) server (available since version 0.7.0), so AI assistants such as Claude Code, Claude Desktop and Cursor can analyze audio and video files for you.
With [uv](https://docs.astral.sh/uv/) installed, you can run it without installing anything else:

```bash
uvx --from "behavioralsignals[mcp]" behavioralsignals-mcp
```
...
claude mcp add --env BEHAVIORALSIGNALS_CID="$BEHAVIORALSIGNALS_CID" \
  ...
  behavioralsignals -- uvx --from "behavioralsignals[mcp]" behavioralsignals-mcp
```
...
```json
{
  "mcpServers": {
    "behavioralsignals": {
      "command": "uvx",
      "args": ["--from", "behavioralsignals[mcp]", "behavioralsignals-mcp"],
      "env": {

...
class Deepfakes(BaseClient):
    def upload_audio(
        ...
        with open(params.file_path, "rb") as audio_file:
            ...
            data = self._send_request(
                path=f"detection/clients/{self.config.user_id}/processes/audio",
                method="POST",
                files=files,
                data=data,
            )
    ...
    def list_processes(
        ...
        data = self._send_request(
            path=f"detection/clients/{self.config.user_id}/processes",
            method="GET",
            data=query_params,
        )
    ...
    def get_process(self, pid: int) -> ProcessItem:
        ...
        data = self._send_request(
            path=f"detection/clients/{self.config.user_id}/processes/{pid}",
            method="GET",
        )
    ...
    def get_result(self, pid: int) -> ResultResponse:
        ...
        """
        data = self._send_request(
            path=f"detection/clients/{self.config.user_id}/processes/{pid}/results",
            method="GET",
        )
        return ResultResponse(**data)

    def stream_audio(self, audio_stream: Iterator[bytes], options: StreamingOptions) -> Iterator[ResultResponse]:
